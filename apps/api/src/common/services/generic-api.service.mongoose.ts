...
export class GenericMongooseApiService<T extends Document & IEntity> {
  ...
  constructor(model: Model<T>, pluralResourceName: string, singularResourceName: string) {
    ...
    this.model.pre('save', this.prepareEntity.bind(this));
  }
   ...
   */
  async getById(userContext: IUserContext, id: string): Promise<T | null> {
  ...
  async create(userContext: IUserContext, entity: Partial<T>): Promise<T | null> {
    ...
    try {
      ...
      const createdEntity = await this.model.create(preparedEntity);
